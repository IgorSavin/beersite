...
import { name, description, dishes, abvLength, ibuLength, srmLength, year, price } from '../../react/selectors/beerPageSelector';
import styles from './BeerPage.css';

function BeerPage({name, description, dishes, abvLength, ibuLength, srmLength, year, price}) {
    return (
            ...
            <div className={styles.beerPage}>
                <div className={styles.beerPage__infoBox}>
                    ...
                    <ul className={styles.beerPage__dishes}>
                        {dishes.map(el => <li className={styles.beerPage__dish}>{el}</li>)}
                    </ul>
                    <div className={styles.beerPage__progressBar}>
                        <div className={styles.beerPage__progressBarBG}><span className={`${styles.beerPage__Value} ${styles.beerPage__abvValue}`} style={{ width: `${abvLength*5}%` }}>abv: {abvLength}</span></div>
                        <div className={styles.beerPage__progressBarBG}><span className={`${styles.beerPage__Value} ${styles.beerPage__ibuValue}`} style={{ width: `${ibuLength*1.15}%` }}>ibu: {ibuLength}</span></div>
                        <div className={styles.beerPage__progressBarBG}><span className={`${styles.beerPage__Value} ${styles.beerPage__srmValue}`} style={{ width: `${srmLength*0.8}%` }}>srm: {srmLength}</span></div>
                    </div>
                </div>
                <div className={styles.beerPage__pictureBox}>
                    {/*<span className={styles.beerPage__star}>&#9734;</span>*/}
                    {/*<span className={styles.beerPage__star}>&#9733;</span>*/}
                    <img src="https://images.punkapi.com/v2/85.png" alt="beer picture"
